...
const SYSTEM_VERSION = '1.9.4-GitHub';
...
window.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter') {
        if (scanBuffer.length > 2) {
            handleSuccessfulScan(scanBuffer);
            scanBuffer = "";
        }
...
function triggerPwaInstall() {
    if (deferredPrompt) {
        ...
        if (isIOS) {
            alert('📱 iPHONE: Clique em COMPARTILHAR e depois em "ADICIONAR À TELA DE INÍCIO".');
        } else {
            alert('📲 ANDROID/ZEBRA: Clique nos 3 PONTINHOS do Chrome e escolha "INSTALAR APLICATIVO" ou "ADICIONAR À TELA INICIAL".');
        }
    }
}
...
const SAMPLE_PRODUCTS = [
    { sku: '7891000200030', productId: '020', productName: 'Dipirona 500mg - Master', lot: 'LT9876-PHARMA', validity: '12/2026', content: '50 cx Dipirona 500mg', address: 'PULMÃO PG15 - Col.3 / Nív.2' },
    { sku: '020', productId: '020', productName: 'Dipirona 500mg - Master', lot: 'LT9876-PHARMA', validity: '12/2026', content: '50 cx Dipirona 500mg', address: 'PULMÃO PG15 - Col.3 / Nív.2' },
            ...
            <div class="label-right">
                <div class="label-row">
                    ...
                    <div>
                        ...
                        <div class="label-field-value" style="font-size: 24px;">${currentEntry.address}</div>
                    </div>
                </div>
                <div class="label-row">
                    <span class="label-icon">➕</span>
                    <div>
                        ...
                        <div class="label-field-value" style="font-size: 24px;">${currentEntry.productName} - Master</div>
                    </div>
                </div>
                <div class="label-row">
                    <span class="label-icon">▌▌▌</span>
                    <div>
                        ...
                        <div class="label-field-value" style="font-size: 24px;">${currentEntry.lot}</div>
                    </div>
                </div>
                <div class="label-row">
                    <span class="label-icon">📅</span>
                    <div>
                        ...
                        <div class="label-field-value" style="font-size: 24px;">${currentEntry.validity}</div>
                    </div>
                </div>
                <div class="label-row">
                    <span class="label-icon">📦</span>
                    <div>
                        ...
                        <div class="label-field-value" style="font-size: 20px;">${currentEntry.content}</div>
                    </div>
                </div>
            </div>
        </div>
...
function startCamera(elementId, silentMode = false) {
    ...
    if (html5QrCode) {
        html5QrCode.stop().then(() => {
            ...
            if (btn) btn.innerText = "📷 ATIVAR CÂMERA REAL";
        }).catch(() => {});
        return;
    }
    ...
    html5QrCode.start(
        ...
        const btn = document.getElementById('start-camera-btn');
        if (btn) btn.innerText = "🛑 PARAR CÂMERA";
        const line = document.querySelector('.scanner-line');
        if (line) line.style.display = 'none';
    }).catch((err) => {
        if (!silentMode) {
            alert("📷 CÂMERA: Ative as permissões ou use HTTPS.");
        }
        console.warn("Scanner: Câmera não iniciada (Modo Bip ativo).");
    });
}
...
function showScreen(screenId, saveHistory = true) {
    ...
    // Stop any active camera before switching screens
    if (html5QrCode) {
        html5QrCode.stop().catch(() => {});
        html5QrCode = null;
    }
    ...
    if (target) {
        ...
        // AUTO-SCANNER LOGIC v1.9.4
        const scanScreens = {
            'screen-phase1': 'reader-phase1',
            'screen-conferencia': 'reader-conf',
            'screen-forklift': 'reader-forklift',
            'screen-stock-query': 'reader-query'
        };

        if (scanScreens[screenId]) {
            // Tenta iniciar a câmera automaticamente (pode falhar no mobile sem gesto, mas tentamos)
            setTimeout(() => {
                startCamera(scanScreens[screenId], true); // true = silent mode
            }, 300);
        }
        ...
        if (screenId === 'screen-phase2') renderLabel();
